    ...
    where T : ItemBase where TDto : ItemBaseDTO where TSpecs : class where TSpecsDto : class
{
    ...
    {
        return _mapper.Map<TDto>(_repository.GetItem(id));
    }

    public TDto GetItemWithInclude(int? id, params Expression<Func<T, object>>[] includeProperties)
    {
        return _mapper.Map<TDto>(_repository.GetItemWithInclude(id, includeProperties));
    }
    ...
    {
        return _mapper.Map<IEnumerable<TDto>>(_repository.GetAllWithInclude(includeProperties));
    }
}